...
import {referenceCatalog,referenceItem} from './catalog.js';
...
let lastSelection=null,previewGeneration=0,previewTimer=null;
...
const Reference=Node.create({
 ...
 addAttributes(){return Object.fromEntries(['kind','id','label','scope','turns','itemId'].map(key=>[key,{default:key==='scope'?'full':''}]));},
...
function insertReference(input,range=null){
 const value=normalizeReference(input),instance=editor.value;
 if(!value||!instance||props.disabled)return;
 ...
 instance.chain().focus().insertContentAt({from:Math.min(from,instance.state.doc.content.size),to:Math.min(to,instance.state.doc.content.size)},[{type:'reference',attrs:value},{type:'text',text:' '}]).run();
 instance.view.dispatch(closeHistory(instance.state.tr));
 closeMention();notify();
}
function onKey(view,event){
 ...
 if(mention.value){
  if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();picker.value?.move(event.key==='ArrowDown'?1:-1);return true;}
  if(event.key==='Enter'||event.key==='Tab'){event.preventDefault();picker.value?.choose();return true;}
  if(event.key==='Escape'){event.preventDefault();closeMention();return true;}
 }
 if(event.key==='Enter'){
...
async function preview(){
 ...
 checking.value=true;
 try{const result=await Api.referencePreview(props.modelValue,props.currentConversation);if(generation===previewGeneration){validation.value=result;editor.value?.storage.reference.invalid.clear();}}
...
onMounted(()=>{
 editor.value=new Editor({
  extensions:[DocumentNode,Paragraph,Text,HardBreak,Reference,Undo],content:textToDocument(props.modelValue),editable:!props.disabled,
  ...
  onUpdate:({editor:instance})=>{emit('update:modelValue',documentToText(instance.getJSON()));nextTick(updateMention);notify();},
...
watch(()=>props.modelValue,value=>{const instance=editor.value;if(instance&&documentToText(instance.getJSON())!==String(value||'')){instance.commands.setContent(textToDocument(value),{emitUpdate:false});lastSelection=null;closeMention();notify();}});
...
defineExpose({focus,adjustHeight,insertText,insertReference,editor});
</script>
<template>
 <div ref="host" class="reference-editor" :class="{'is-drag-over':dragHover,'has-reference-error':validation?.ok===false}" @dragover.capture="dragOver" @dragleave="dragHover=false" @drop.capture="drop">
  ...
  <ReferencePicker ref="picker" :open="Boolean(mention)" :anchor="mention?.anchor" :query="mention?.query||''" :kind="mention?.kind||''" :current-conversation="currentConversation" @select="insertReference" @close="closeMention"/>
 </div>
</template>
<style>
