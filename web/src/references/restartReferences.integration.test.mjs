...
const resetStart=source.indexOf('function applyCommittedTurnDeletion(');
const resetEnd=source.indexOf('function clearUiCaches()',resetStart);
assert.ok(resetStart >= 0 && resetEnd > resetStart, 'extract the real committed deletion reset');
...
function harness({draft='',confirm=async()=>{},remove=async()=>({deletedRootTurns:['turn']}),load=async()=>{}}={}){
 const calls={deleted:[],saved:[],closed:0,loaded:0,resets:0,errors:[]};
 const turn={user:{turnUuid:'turn',content:original,attachments:[]}};
 let operations=[{opId:'survivor',turnUuid:'safe',runRootTurnId:'safe'}, {opId:'deleted',turnUuid:'turn',runRootTurnId:'turn'}];
 const ctx=vm.createContext({componentMounted:true,activeConversationUuid:ref('current'),isLocalConversation:ref(false),deletingTurnUuid:ref(''),running:ref(false),turns:ref([turn]),draft:ref(draft),draftEditRevision:0,lastFrameSeq:ref(1),plainText:x=>x,
  ...
  closeWs:()=>calls.closed++,resetOperationStore:()=>{calls.resets++;operations=[];},loadRequestGeneration:0,
  orderedOperationsList:()=>operations,replaceOperationSnapshots:next=>{operations=next;},
  projectOperationMessages:next=>next.map(op=>({role:'user',content:op.opId})),
  messages:ref([{role:'user',content:'survivor'},{role:'user',content:'deleted'}]),chatState:ref({operations}),
  foregroundRunning:ref(false),rootTurnRunning:ref(false),lastStats:ref(null),runStartedAt:ref(0),status:ref(''),clearActiveRun(){},
  toolDetailCache:{reset(){}},load:async()=>{calls.loaded++;await load();},emit(){},
 ...
 vm.runInContext(resetCode+code,ctx);
...
test('committed deletion clears the removed turn before its fresh HTTP state arrives',async()=>{
 const pending=deferred(),h=harness({load:()=>pending.promise});
 try{
  const job=h.run();await flush();
  assert.equal(h.calls.resets,1);
  assert.deepEqual(h.ctx.messages.value.map(m=>m.content),['survivor']);
  assert.deepEqual(h.ctx.chatState.value.operations.map(op=>op.opId),['survivor']);
  pending.resolve();await job;
 }finally{h.stop();}
});
