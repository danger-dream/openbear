...
const props=defineProps({open:Boolean,anchor:{default:null},placement:{type:String,default:'right-start'},width:{type:Number,default:340},label:{type:String,default:'引用资源'}});
...
async function position(){
 ...
 panel.value.style.width=`${width}px`;panel.value.style.maxHeight=`${Math.min(430,availableHeight)}px`;
 ...
 style.value={position:'fixed',left:`${pos.x}px`,top:`${Math.max(8,top)}px`,width:`${width}px`,maxHeight:`${Math.min(430,availableHeight)}px`,visibility:'visible'};
}
...
function key(event){if(event.key==='Escape')emit('close');}
watch(()=>[props.open,props.anchor],async()=>{resizeObserver?.disconnect();positionGeneration++;if(props.open&&typeof window!=='undefined'){await nextTick();schedule();if(typeof ResizeObserver!=='undefined'&&panel.value){resizeObserver=new ResizeObserver(schedule);resizeObserver.observe(panel.value);}}},{immediate:true});
...
<template><Teleport to="body"><section v-if="open" ref="panel" class="reference-floating-panel" :style="style" :aria-label="label" @pointerenter="emit('enter')" @pointerleave="emit('leave')" @click.stop><slot/></section></Teleport></template>
<style>
