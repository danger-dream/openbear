...
export const Api = {
  ...
  referencePreview: (text, conversationUuid = "", existingKeys = []) => api.post("/references/preview", { text, conversationUuid, existingKeys }).then(unwrap),
  referenceInspect: (text, conversationUuid = "", bundleId = "") => api.post("/references/inspect", { text, conversationUuid, bundleId }).then(unwrap),
  referenceHistory: (uuid, offset = 0, view = '') => api.get(`/reference-history/${encodeURIComponent(uuid)}`, { params: { offset, ...(view ? { view } : {}) } }).then(unwrap),
  ...
  setMcpServerAgentAccess: (serverKey, agentAccess, expectedAgentAccess) => api.patch(`/mcp/servers/${encodeURIComponent(serverKey)}/agent-access`, { agentAccess, expectedAgentAccess }).then(unwrap),
  refreshMcpServerTools: (serverKey) => api.post(`/mcp/servers/${encodeURIComponent(serverKey)}/refresh-tools`, {}, { timeout: 120000 }).then(unwrap),
