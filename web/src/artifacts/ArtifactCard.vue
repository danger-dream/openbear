...
import {artifactFromUrl, artifactRecord, artifactFormat, formatFileSize, loadArtifactCard, openArtifactPreview} from "./artifactFiles.js";
...
<template>
	<div ref="root" class="artifact-card" :class="{'is-unavailable': record?.metadataError}" :data-artifact-id="identity?.artifactUuid">
...
.artifact-card { position: relative; width: 100%; overflow: hidden; border: 1px solid var(--ob-border-soft, #e1e5eb); border-radius: 14px; background: var(--ob-surface, #fff); color: var(--ob-text, #303641); box-shadow: 0 2px 5px rgb(20 30 50 / 3%); transition: border-color .15s, box-shadow .15s; }
.artifact-card:hover { border-color: var(--ob-blue, #a6bbd6); box-shadow: 0 4px 14px rgb(20 30 50 / 6%); }
...
.artifact-card-icon { display: grid; place-items: center; width: 42px; height: 48px; border: 1px solid var(--ob-border-soft, #dce7f1); border-radius: 10px; color: var(--ob-blue, #577fa9); background: var(--ob-surface-raised, #f0f5fa); }
.artifact-card-icon svg { width: 25px; height: 25px; }
.artifact-card-copy { display: flex; min-width: 0; flex-direction: column; gap: 6px; }
.artifact-card-title { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--ob-text-strong, #263343); font-size: 14px; font-weight: 650; line-height: 1.6; }
.artifact-card-excerpt { display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; height: 37px; color: var(--ob-text-subtle, #78808d); font-size: 12px; line-height: 18.5px; overflow-wrap: anywhere; }
.artifact-card-meta { display: flex; align-items: center; gap: 7px; padding-top: 1px; font-size: 10.5px; font-weight: 500; line-height: 16px; color: var(--ob-text-muted, #9199a5); }
.artifact-card-download { position: absolute; top: 12px; right: 11px; display: grid; place-items: center; width: 32px; height: 32px; border-radius: 8px; color: var(--ob-text-subtle, #7f8997) !important; text-decoration: none !important; }
.artifact-card-download:hover { background: var(--ob-surface-soft, #edf2f7); color: var(--ob-blue, #3b6b9b) !important; }
.artifact-card-download svg { width: 18px; height: 18px; }
.artifact-card-open:focus-visible, .artifact-card-download:focus-visible { outline: 2px solid var(--ob-blue, #729ac3); outline-offset: -3px; }
.artifact-card.is-unavailable .artifact-card-excerpt { color: var(--ob-text-muted, #959ba5); }
