...
- 在「设置 → 参数设置 → 浏览器」启用并配置。一个原生工具覆盖页面操作、快照、截图、文件和网络；由 Python 直接通过 CDP 执行，无需 Node/npm、Playwright 驱动或任何浏览器 MCP 服务。独立 CDP 管理通道与可回收的 Python worker 分离。
- 默认使用已有登录浏览器，也可选择独立临时实例。连接、视口、超时、输出容量及恢复策略均可配置；升级不会自动接管现有页面。
- 浏览器内辅助算法随包分发，安装/升级仅校验资源，不下载浏览器或 npm 依赖。远端浏览器下载可配置共享目录。配置与使用说明见 [原生 Browser 文档](docs/browser.md)。

### MCP 与 Agent 访问

- 在「MCP 管理 → 服务详情 → 允许 Agent 访问」设置每个服务：**关闭（默认）**、**全部工具**或**指定工具**。旧配置、现有已信任服务和新安装均不会自动开放给 Agent；主控的访问和原有调用审批不变。
- 全部模式包含未来新增工具；指定模式按原始工具名精确匹配，不支持通配符。「全选当前工具」只保存当前名单。断连或工具消失时保留选择，恢复后仍受当前授权约束。
- 开放只是上限：创建或续接 Agent 必须传完整 `tools` 数组。预设空名单不附加限制，但 `tools=[]` 表示本轮无业务工具；Plan 仍须申请、批准并开始步骤。撤权阻挡下一次未发出的调用，不重放在途请求。
- 每张服务卡的「刷新工具」经**该服务现有连接**重新读取完整工具列表，同时更新同名工具说明和参数枚举；不重连其他服务。失败保留原清单。后续主控轮次、新 Agent 和 Continue 使用新 Schema；执行中的 Plan 保留本轮契约，但实时撤权优先。
- 单纯访问策略保存或仅该策略的配置热加载不重连 MCP。配置字段位于 `mcp.servers[服务key].agentAccess`，例如 `{"mode":"selected","tools":["search","read_document"]}`；缺省等同 `{"mode":"disabled","tools":[]}`。
- 使用 MCP 原生公开工具名和完整参数，不再提供旧内置 `WebSearch` / `WebExtract` 包装；旧名称不会自动绑定到任何 MCP，也不会自动为旧预设开放服务。请按需显式设置 MCP 和预设。

### 从 v0.6.0 升级

- 旧配置无需补填 `agentAccess`；现有 MCP 对 Agent 默认仍关闭。升级不会自动替换数据库中的自定义主控／Agent 模板。
- 旧 Agent 预设中的 `WebSearch` / `WebExtract` 会以“当前不可用”选择保留，不会自动映射到 MCP 或获得授权；仅含旧工具的预设也不会被当成“不限制工具”的空预设。要恢复搜索，请先在 MCP 管理中显式开放目标服务，再在 Agent 设置中选择其原生公开工具名并移除旧项。
- 如需新版 MCP 工具目录与服务说明，可在「提示词模板」中“导入随版本模板”，审核后手动激活。现有会话快照和 AgentContinue 实例仍保留旧模板；新模板需在新会话或新 Agent 实例中生效。自定义模板中硬编码的旧工具名称应由用户按实际 MCP 配置调整。
- 旧文本草稿继续保留。v0.6.0 尚未发送的附件仅存于页面内存，不能跨刷新迁移；升级刷新前请先发送，或保留原文件以便重新添加。新版新增的附件草稿支持持久恢复。
...
发行包已包含前端构建产物，运行 OpenBear 及原生 Browser 均不需要 Node。只有源码构建缺少前端产物时，安装器才为前端构建准备 Node；用户另行配置的 MCP 服务依赖按各自要求准备。
...
正式发布在维护者本机 Docker 中先完成 Python 3.11／3.12／3.13 后端全测和一次前端全测，再构建、打包；同一 ZIP 通过真实 systemd 干净安装及上一正式版升级验收后才上传。发行包登录验证使用 HTTP/WebSocket，不运行浏览器自动化。GitHub 仅托管源码、tag 和已验收资产，推送 tag 不再触发另一套测试、构建或发布。
